...
export default function KargoAyarlariClient({ settings }: { settings: any }) {
    ...
    const [shippingFee, setShippingFee] = useState(formatNum(settings?.shippingFee));
    const [currency] = useState(settings?.currency || "TL");
    const [applyToFree, setApplyToFree] = useState(!!settings?.applyToFree);
    const [calcMethod, setCalcMethod] = useState<ShippingCalcMethod>(settings?.calcMethod as ShippingCalcMethod || "single");
    const [calcMenuOpen, setCalcMenuOpen] = useState(false);

    // Hızlı Teslimat Bildirimi YERİNE sadece var olan DB fieldları kalacak
    const [freeShipping, setFreeShipping] = useState(!!settings?.freeShippingActive);

    // internationalActive, kullanicinin istedigi gibi "silik" olacak, DB'de false olarak update edilecek
    const international = false;

    // first_plus states
    const [extraPerItem, setExtraPerItem] = useState(formatNum(settings?.firstPlusExtra));

    // threshold states
    const [thresholdValue, setThresholdValue] = useState(formatNum(settings?.thresholdValue));
    const [belowThresholdFee, setBelowThresholdFee] = useState(formatNum(settings?.belowThresholdFee));
    const [aboveThresholdFee, setAboveThresholdFee] = useState(formatNum(settings?.aboveThresholdFee));

    // delivery_method states
    const [selectedCarrier, setSelectedCarrier] = useState("");
    const [carrierDropdownOpen, setCarrierDropdownOpen] = useState(false);
    const [deliveryMethods, setDeliveryMethods] = useState<DeliveryMethod[]>(
        (settings?.deliveryMethods as DeliveryMethod[]) ||
    ...
    const handleSave = async () => {
        ...
        const payload = {
            shippingFee: parseFloatSafe(shippingFee),
            applyToFree,
            calcMethod,
            freeShippingActive: freeShipping,
            firstPlusExtra: parseFloatSafe(extraPerItem),
            thresholdValue: parseFloatSafe(thresholdValue),
            belowThresholdFee: parseFloatSafe(belowThresholdFee),
            aboveThresholdFee: parseFloatSafe(aboveThresholdFee),
            deliveryMethods
        };
        ...
        if (res.success) {
            showToast(res.message || "Başarılı", "success");
        } else {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Yurt Dışına Gönderim (Disabled & Silik) */}
            <div className="bg-gray-50 rounded-xl border border-[#E5E7EB] overflow-hidden opacity-60">
                <div className="px-6 py-4 border-b border-[#E5E7EB] flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-[#9CA3AF]">Uluslararası gönderim seçeneklerini yönetin (Anlaşma Kapsamında Değil)</p>
                    </div>
                </div>
                <div className="p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            disabled
                            className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors flex-shrink-0 ml-6 bg-[#D1D5DB] cursor-not-allowed`}
                        >
                            <span
                                className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform translate-x-1`}
                            />
                        </button>
                    </div>
                    <div className="mt-4">
                        <span className="text-xs text-[#DC2626] bg-[#DC2626]/10 px-3 py-2 rounded font-medium">
                            İhtiyaç halinde yazılımcı tarafından aktif edilecektir.
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-end">
                <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className="flex items-center gap-2 bg-[#FF007F] text-white px-8 py-3 rounded-lg font-medium text-sm hover:bg-[#D6006B] transition-colors shadow-lg shadow-[#FF007F]/20 disabled:opacity-60"
                >
                    <span className="material-icons text-xl">{isSaving ? "sync" : "save"}</span>
