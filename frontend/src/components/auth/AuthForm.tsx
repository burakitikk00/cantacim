...
export default function AuthForm({ onSuccess, isModal = false }: AuthFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isLogin) {
                ...
                if (res?.error) {
                    if (res.status === 429) {
                        setError("Çok fazla deneme yaptınız. Güvenliğiniz için 15 dakika bekledikten sonra tekrar deneyiniz.");
                    } else {
    ...
    return (
        <div className="w-full max-w-md space-y-8">
                ...
                {!isLogin && (
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="name" value={formData.name} onChange={handleChange} required className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:ring-1 focus:ring-primary focus:border-primary" placeholder="Adınız" />
                        </div>
                        <div>
                            <label className="text-xs font-bold uppercase tracking-widest text-primary/60 block mb-2">Soyad</label>
                            <input name="surname" value={formData.surname} onChange={handleChange} required className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:ring-1 focus:ring-primary focus:border-primary" placeholder="Soyadınız" />
                        </div>
                    </div>
                )}
                <div>
                    <label className="text-xs font-bold uppercase tracking-widest text-primary/60 block mb-2">E-posta</label>
                    <input name="email" type="email" value={formData.email} onChange={handleChange} required className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:ring-1 focus:ring-primary focus:border-primary" placeholder="ornek@email.com" />
                </div>
                <div>
            ...
            <button type="button" onClick={() => signIn("google", { callbackUrl })} className="w-full border border-gray-200 py-4 rounded-lg font-medium text-sm hover:bg-gray-50 transition-colors flex items-center justify-center gap-3">
                {/* SVG path omitted for brevity, can re-add if needed */}
